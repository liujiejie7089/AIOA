...
- **用户口径确认**：知识库存储 = **Milvus**（不是 mysql），嵌入 = **local**（256 维），**不部署 Ollama**。这与 §7 的「本次部署档位」一致，无需改代码；生产档 `deploy/.env.production` 已是 `AIOA_KB_STORE=milvus` + `AIOA_MILVUS_URI=http://10.0.0.12:19530`（**2026-09-24 更正：Milvus 在 10.0.0.12、与应用机不同机**）+ `AIOA_MILVUS_DIMS=256` + 集合 `kb_chunk_v1_256`。
...
- **本机无法验证生产 Milvus**：`10.0.0.12:19530` 从开发机 `timeout 6 bash -c 'cat < /dev/tcp/10.0.0.12/19530'` **不可达**（超时；2026-09-24 复测仍不可达 —— 正常，链路是「应用机 → Milvus」）⇒ `store=milvus` 的活体自建集合/混合检索/回填仍未实测，判据与补验步骤见 §7 与 `docs/32 §11.3`。本机 :8080 跑的是 `store=mysql`。
