...
| 变量 | 用途 | 本地开发 | 生产 | 敏感 |
|---|---|---|---|---|
| `SPRING_DATASOURCE_URL` | 后端 JDBC 连接串 | `jdbc:mysql://127.0.0.1:3306/aioa?…` | `jdbc:mysql://mysql:3306/aioa?…` | — |
| `SPRING_DATASOURCE_USERNAME` / `_PASSWORD` | 后端库账号 | `root` / 空（本机无密码） | `<db-user>` / `CHANGE_ME__` | ★ |
| `MYSQL_HOST` / `_PORT` / `_DB` | compose 起库与拼串 | `127.0.0.1` / `3306` / `aioa` | `mysql` / `3306` / `aioa` | — |
| `MYSQL_USER` / `MYSQL_PASSWORD` / `MYSQL_ROOT_PASSWORD` | compose 库凭据 | `root` / 空 / `DEV_ONLY__` | 均 `CHANGE_ME__` | ★ |
| `MYSQL_DATABASE` | 初始化库名 | `aioa` | `aioa` | — |

### 3.2 Redis

| 变量 | 用途 | 本地开发 | 生产 | 敏感 |
|---|---|---|---|---|
| `SPRING_REDIS_HOST` / `_PORT` | 后端连 Redis | `127.0.0.1` / `6379` | `redis` / `6379` | — |
| `SPRING_REDIS_PASSWORD` | 密码 | 空 | `CHANGE_ME__` | ★ |
| `SPRING_REDIS_DATABASE` | 库号 | `0` | `0` | — |
| `REDIS_HOST` / `REDIS_PORT` | compose 侧 | `127.0.0.1` / `6379` | `redis` / `6379` | — |
...
| `AIOA_MILVUS_URI` | Milvus gRPC 地址 | `http://127.0.0.1:19530` | `http://milvus:19530` | — |
| `AIOA_MILVUS_TOKEN` | 鉴权令牌（单机未开鉴权时留空） | 空 | 空 | ★ |
