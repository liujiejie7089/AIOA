...
public class GiteeTokenService {
     ...
     *
     * @throws BizException 未绑定或已失效（提示用户重新绑定）
     */
    ...
    public String validToken(GiteeAccount acc) {
        ...
        if (!StringUtils.hasText(token)) {
            throw BizException.badRequest("Gitee 绑定缺少 access_token，请重新授权");
        }
        ...
        synchronized (locks.computeIfAbsent(acc.getUserId(), k -> new Object())) {
            ...
            if (fresh == null) {
                throw BizException.badRequest("Gitee 绑定已解除，请重新授权");
            }
    ...
    private String refresh(GiteeAccount acc) {
        ...
        if (!StringUtils.hasText(rt)) {
            throw BizException.badRequest("Gitee 授权已过期且无 refresh_token，请重新授权");
        }
        ...
        try {
            ...
            throw BizException.badRequest("Gitee 授权已失效，请重新授权绑定：" + e.getMessage());
        }
        String newAccess = str(resp.get("access_token"));
        String newRefresh = str(resp.get("refresh_token"));
        if (!StringUtils.hasText(newAccess)) {
            throw BizException.badRequest("Gitee 刷新令牌未返回 access_token，请重新授权");
        }
    ...
    public void assertEnabled() {
        if (!props.isEnabled()) {
            throw BizException.badRequest("Gitee 联动未启用：请配置 aioa.gitee.enabled=true 与 client_id/client_secret");
        }
    }
